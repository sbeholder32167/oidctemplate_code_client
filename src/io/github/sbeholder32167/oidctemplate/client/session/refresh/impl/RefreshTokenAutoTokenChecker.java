...
/**
 * Refresh Token 만료 시각 기준 갱신 정책 구현체 Class.<br>
 *
 * <p>Refresh Token의 만료 시각을 기준으로 Token Refresh 필요 여부를 판단<br>
 ...
 */
//-- XML Bean 등록
public class RefreshTokenAutoTokenChecker implements OIDCAutoTokenChecker {
    @Override
    public boolean needToRefreshTokens(OIDCSession oidcSession, int idpTimeoutSec, int skewSec) {
        ...
        if (oidcSession.getRefreshTokenExpirationTimeSec() - skewSec < currentTimeSec) {
            LogUtil.info((oidcSession.getRefreshTokenExpirationTimeSec() - skewSec) + "/" + currentTimeSec, this);
            return true;
        } else {
            return false;
        }
