...
/**
 * Access Token 만료 시각 기준 갱신 정책 구현체 Class.<br>
 *
 * <p>Access Token의 만료 시각을 기준으로 Token Refresh 필요 여부를 판단한다.<br>
 ...
 */
//-- XML Bean 등록
public class AccessTokenAutoTokenChecker implements OIDCAutoTokenChecker {
    @Override
    public boolean needToRefreshTokens(OIDCSession oidcSession, int idpTimeoutSec, int skewSec) {
        ...
        if (oidcSession.getAccessTokenExpirationTimeSec() - skewSec < currentTimeSec) {
            LogUtil.info("Session:" + (oidcSession.getAccessTokenExpirationTimeSec() - skewSec) + " / Current:" + currentTimeSec, this);
            return true;
        } else {
            return false;
        }
